...
Creates a new `Texture2D` of the given dimensions filled with a single solid color. Used internally by `GlobalStyles` to generate the background textures for every UI element.
...
A helper method on `GlobalStyles` that stamps a `Texture2D` onto all interactive states (`normal`, `hover`, `active`, `onNormal`, `onHover`, `onActive`) of a given `GUIStyle` at once — saving repetitive property assignment when defining a new style.

```csharp
// Apply the same texture to every state of a custom style
GlobalStyles.ApplyBackground(ref myStyle, GlobalTex.MakeTex(1, 1, new Color(0.2f, 0.2f, 0.2f, 1f)));
```
...
2. If the dictionary marks this `id` as open, renders a button for every entry in `options` below the main button.
...
InputSelectors.PSelectedIndex = MenuHelper.Dropdown(
    ...
    new[]
    {
        ...
        "A Button", "B Button",
        "X Button", "Y Button",
        "Right Joystick Click", "Left Joystick Click"
    },
